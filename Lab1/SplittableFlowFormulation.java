...
public class SplittableFlowFormulation implements IAlgorithm
{
	...
	op.addDecisionVariable("x_lkc" , false , new int [] {D , E } , 0 , Double.MAX_VALUE);	//lkc: link identifier and node pair identifier
	...
	for (Node n : netPlan.getNodes ())
	{
		...
		{
			op.setInputParameter ("c" , d.getIndex ());		//c: node pair
			op.setInputParameter ("v_c" , d.getOfferedTraffic());
			if (n == d.getIngressNode())
				op.addConstraint ("sum(x_lkc(c,OutgoingLinks)) - sum(x_lkc(c,IncomingLinks)) == v_c");		//constraints
