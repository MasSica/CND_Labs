...
public class RouteFormulationWP implements IAlgorithm
{
	...
	public String executeAlgorithm(NetPlan netPlan, Map<String, String> algorithmParameters, Map<String, String> net2planParameters)
	{
		...
		op.addDecisionVariable("r_cnw" , isNonBifurcated , new int [] {1 , netPlan.getNumberOfRoutes (),W } , 0 , Double.MAX_VALUE);  // fraction between 0 and 1
		...
		for (Demand d : netPlan.getDemands ())
		{
			...
			for(int w=0;w<W;w++) {
				...
				op.addConstraint ("sum(r_cnw(0,n,w)) == v_cw(c,w)");
				//System.out.println(op.addConstraint ("sum(r_cnw(0,n,w)) == v_cw(c,w)").toString());
			}
			op.addConstraint ("sum(v_cw(c,all)) == V_c");
			//System.out.println(op.addConstraint ("sum(v_cw(c,all)) == V_c").toString());
		}
		...
		for (Link e : netPlan.getLinks ())
		{
			...
			op.addConstraint ("sum(r_cnw(0,R_lk,all)) <= linkcapacity");
			//System.out.println(op.addConstraint ("sum(r_cnw(0,R_lk,all)) <= linkcapacity").toString());
			//}
		}
		...
		for (Route r : netPlan.getRoutes())
		{
			...
			//for(Demand d : netPlan.getDemands ())
				for(int w=0;w<W;w++) {
				traffic=traffic+r_cnw[0][index][w];	//d.getIndex()
			}
